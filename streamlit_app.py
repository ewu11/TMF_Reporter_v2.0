...
def process_messages_from_file(file_contents):
    ...
    issue_patterns = {
        ...
        "Slotted HSBA Order, Remained Returned": r"\bo(.)?der.*slot(ted)?.*portal.*tm(.)?f(orce)?.*return(ed)?\b|\bo(.)?der.*slot(ted)?.*portal.*tapi.*tm(.)?f(orce)? m(.)?s(.)?h status\b|\bbantu(.)?(.)?.*slot(kan)?.*(appt|appmnt|ap(.)?(.)?(.)?(.)?(.)?(.)?(.)?(.)?(.)?).*asal\b|\ba(.)?(.)?ist to ch(.)?(.)?g(.)? to or(.)?g(.)?(.)?(.)?(.)? timing for below o(.)?der(.)?\b|\brequest to slot as original date\b",
