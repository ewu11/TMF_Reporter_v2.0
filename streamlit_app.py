...
def process_messages_from_file(file_contents):
    ...
    issue_patterns = {
        ...
        "TT Missing": r"\bada (dalam|dlm|dekat|dkt) nova (tapi|tp) (tiada|xda|takda) (dalam|dlm|dekat|dkt) tmf\b|\b(retrigger|trigger) ctt\b|\bada dlm nova tp x de dlm tmf\b|\bctt missing\b|\bctt tiada dalam tmf\b|\bm(o)?h(o)?n (re)?(-)?trigger.*(missing|h(i)?l(a)?(n)?g) d(a)?l(a)?m (act(ivity)?|xtvt|aktiviti) list\b|\btt.*appear(kan)? d(a)?l(a)?m tm( )?f(orce)?\b|\b(c)?tt h(i)?l(a)?(n)?g.*tm( )?f(orce)?\b|\b(.)?tt.*m(.)?s(.)?k.*tm( )?f(orce)?\b|\b((.)?tt)?.*er(.)?(.)?(.)? 500\b",
        "TT Update DiagnosisCode": r"\bdiagnosis( missing| unsync)\b|\b(rno|fs) troubleshooting\b",
        "TT Granite Network Info Error": r"\bcamelia detect data no found\b|\b(tidak|tak|x) dapat pas(s)?( ke)? next\b|\b(c)?tt (x( )d(a|e)|tiada) (m(.)?kl(.)?m(.)?t|info) granite\b",
        "TT HSBA Reappointment": r"\bappt( ctt)? hsba\b|\b(1-[8|9]\d{10,11})?.*patch( )?( )?(appointment|appt|ctt).*(1-[8|9]\d{10,11})?\b|\bpatch.*a(p)?(p)?(o)?(i)?(n)?(t)?(m)?(e)?(n)?(t)?.*(\d{2}/\d{2}/\d{4})\s*@\s*(\d{1,2}:\d{2}:\d{2}\s*[ap]m)\b|\b.patch.*a(p)?(p)?(o)?(i)?(n)?(t)?(m)?(e)?(n)?(t)?.*\d{2}/\d{2}/\d{4} \d{1,2}:\d{2}:\d{2} (am|pm)\b",
        "TT Invalid Activity Type": r"\b(ac|x)t(i)?v(i)?t(y)? t(y)?p(e)? (s(a)?l(a)?h|inv(a)?lid)( )?(route|l(e)?t(a)?k|assign(ed)?)?\b",
        "Resource Management Issue": r"\bsalah zone id\b|\b(manual task|mt) (t(a)?k|x)( )?d(a)?p(a)?t( nak)?( )?assign\b|\ba(s)?(s)?(i)?(g)?(n)? (mt|m(a)?(n)?(u)?(a)?l t(a)?(s)?(k)?)\b|\b(t(a)?k|x)( )?b(o)?l(e)?h upd(a)?(t)?(e)? c(u)?t(i)?\b",
        "TT Duplicated Activity": r"\bc(a)?(n)?c(e)?l dup(licate(d)?)? act(i)?(v)?(i)?(t)?(y)?\b|\bcancel satu a.t(ivity)? a-\d{9,10}\b",
        "TT Duplicate CPE SN#": r"\bdup(licate(d)?)? item.*(n(o)?(m)?(b)?(o)?(r)?|n(u)?m(b)?(e)?(r)?) (siri|sn(#)?).*s(a)?m(a|e)\b|\b(n(o)?mb(o)?r|n(u)?mb(e)?r) (siri|sn(#)?).*s(a)?m(a|e)\b|\b(c)?tt.*s(\/)?n dup(licate)?\b|\bdup(licate)? (serial no|sn)( #)?( )?faulty\b|\bduplicate sn( untuk .. (dan|&)( ....)?)?\b|\b(n(o|u)(m)?)(b(e|o)r)? (sn|serial number) s(.)?m(.)?\b"
    }
    ...
    for message in messages:
        ...
        for issue, pattern in issue_patterns.items():
            if re.search(pattern, message, re.IGNORECASE):
                ...
                elif issue == "Order Missing/ Pending Processing" or issue == "Missing Manual Assign Button" or issue == "Patch Combo Flag (AX30002_5G no stock)" or issue == "TT RG6/ Combo Update" or issue == "TT Missing/ Update Network Details" or issue == "TT V1P" or issue == "Release Assign To Me":
                    if tickets:
