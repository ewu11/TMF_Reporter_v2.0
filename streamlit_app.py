...
global_result = {
    ...
    "TaaS PPOE Info Missing": [],
    "SLIMS - CPE Transfer Issue": [],
    "TOS Done, SI Unsync": [],
    "TaaS HSI Inactive": [],
...
def process_messages_from_file(file_contents):
    ...
    issue_patterns = {
        ...
        "Resource Management Issue": r"\bsalah zone id\b|\b(manual task|mt) (t(a)?k|x)( )?d(a)?p(a)?t( nak)?( )?assign\b|\ba(s)?(s)?(i)?(g)?(n)? (mt|m(a)?(n)?(u)?(a)?l t(a)?(s)?(k)?)\b|\b(t(a)?k|x)( )?b(o)?l(e)?h upd(a)?(t)?(e)? c(u)?t(i)?\b|\bmasuk((.)?an)?.*ke(.)?d(.)?l(.)?m.*id\b|\bm(.)?s(.)?(.)? (t(.)?k|x) b(.)?l(.)?h.*a(.)?(.)?ig(.)?.*(id|ui|ru)\b|\bteam.*(access|akses)*tm(.)?f(orce)?\b|\bl(.)?p(.)?s t(.)?k(.)?n unlock k(.)?l(.)?(.)?r m(.)?c(.)?m ni(.)?\b|\bbook.*(tiada|x(.)?d(.)? slot)\b",
        "TT Duplicated Activity": r"\bc(a)?(n)?c(e)?l dup(licate(d)?)? act(i)?(v)?(i)?(t)?(y)?\b|\bcancel satu a.t(ivity)? a-\d{9,10}\b|\bcancel.*act\b|\bcancel(led)?.*duplicate(.)?.*(activity|xtvt|aktiviti)\b|\bada (2|dua) (act(.)?(.)?(.)?(.)?(.)?(.)?|akt(.)?(.)?(.)?(.)?(.)?|xtvt)\b|\b(.)?tt.*dup(.)?(.)?(.)?(.)?(.)?(.)?.*(activity|aktivity|(.)?(.)?(.)?(.)?)\b|\bcancel (act|akt|activity|aktivity|xtvt).*pending a(.)?(.)?i(.)?(.)?\b|\bcancel(led)?(.)?(act|akt|xtvt|activity|aktiviti).*dup(.)?(.)?(.)?(.)?(.)?(.)? (.)?tt\b|\bremove or cancel(.)?(.)?(.)? a(ct|k)(.)?(.)?(.)?(.)?(.)?\b|\b(.)?tt.*(mempunyai|ada) 2 waktu (app|appointment).*(lain|berbeza)\b|\bcancel salah satu (ac(.)?(.)?v(.)?(.)?(.)?|ak(.)?(.)?(.)?(.)?(.)?(.)?|xtvt) id\b|\b(act(ivity)?|akt(iviti)?|xtvt) dup(licate(.)?)?.*cancel.*satu\b|\bcancel(.)?(.)?(.)? (activity|act|xtvt|actvty).*dup(.)?(.)?(.)?(.)?(.)?(.)?(.)?\b|\bremove s(.)?l(.)?h satu (act|akt|xtvt|acvty|activity|aktiviti)\b|\bcancel(.)?(k(.)?n)?.*(act|akt|xtvt|acvty|activity|aktiviti)( (.)?tt)?\b|\bcancel.*(activit(.)?|act|aktivit(.)?).*(.)?tt.*s(.)?m(.)?\b",
        ...
        "TT Expired CPE Warranty": r"\bb(.)?t(.)?l(k(.)?n)?.*warranty.*cpe.*(c)?tt b(.)?r(.)?.*(c)?tt l(.)?m(.)?\b|\b(.)?tt.*cpe b(.)?l(.)?m upd((.)?(.)?(.)?)? wa(.)?(.)?anty\b",
        "Invalid TT Info": r"\b(s(.)?m(.)?k|ch(.)?ck) (.)?tt.*s(.)?l(.)?h create.*( create(.)?.*vobb.*s(.)?m(.)?.*c(.)?st(.)?m(.)?r.*(lain|berbeza))?\b",
        "Invalid Order Info": r"\bftc.*t(.)?k(.)?r.*service.*t(.)?(.)?(.)?p(.)?.*m(.)?s(.)?(.)? detect.*l(.)?m(.)?\b",
        "TaaS AOS Not Updated": r"\bstat(us)? (aos|activate(.)?on(.)?site) - n\b",
        "TaaS PPOE Info Missing": r"\bppoe (x(.)?d(.)?|tiada|xdk|t(.)?d(.)?k ad(.)?) d(.)?l(.)?m tm(.)?f(orce)?\b",
        "SLIMS - CPE Transfer Issue": r"\b(x|tidak) boleh tutup cpe\b|\b(t(.)?k|x|t(.)?d(.)?k)(.)?d(.)?p(.)?t (xfer|tr(.)?nsf(.)?r) ke id\b|\berr(.)?(.)? fail(.)?(.)?.*(xfer|tra(.)?(.)?fer) cpe\b",
        "TOS Done, SI Unsync": r"\b(..) (d(.)?h|done) tos.*in(.)?progres(.)?.*tos.*a(.)?sig(.)?(.)?(.)?\b|\b(..).*b(.)?l(.)?(.)?.*on(.)?site.*on my way.*l(.)?p(.)?s\b",
        "TaaS HSI Inactive": r"o(.)?d(.)?(.)? new( install)?.*hsi.*up"
    }
