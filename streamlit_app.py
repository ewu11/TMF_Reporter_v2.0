...
def process_messages_from_file(file_contents):
    ...
    issue_patterns = {
        ...
        "TT Duplicated Activity": r"\bc(a)?(n)?c(e)?l dup(licate(d)?)? act(i)?(v)?(i)?(t)?(y)?\b|\bcancel satu a.t(ivity)? a-\d{9,10}\b|\bcancel.*act\b|\bcancel(led)?.*duplicate(.)?.*(activity|xtvt|aktiviti)\b",
