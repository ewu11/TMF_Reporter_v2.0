...
def process_messages_from_file(file_contents):
    ...
    ticket_order_pattern = r'\b1-\d{9,13}\b|\bT-\d{9}\b|\bt-\d{10}\b|\b1-[a-z0-9]{7}\b|\binc\b|\b25\d{14}\b'  # Ticket or order numbers
    ...
    issue_patterns = {
        ...
        "Resource Management Issue": r"\bsalah zone id\b|\b(manual task|mt) (t(a)?k|x)( )?d(a)?p(a)?t( nak)?( )?assign\b|\ba(s)?(s)?(i)?(g)?(n)? (mt|m(a)?(n)?(u)?(a)?l t(a)?(s)?(k)?)\b|\b(t(a)?k|x)( )?b(o)?l(e)?h upd(a)?(t)?(e)? c(u)?t(i)?\b|\bmasuk((.)?an)?.*ke(.)?d(.)?l(.)?m.*id\b|\bm(.)?s(.)?(.)? (t(.)?k|x) b(.)?l(.)?h.*a(.)?(.)?ig(.)?.*(id|ui|ru)\b",
