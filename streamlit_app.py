...
def process_messages_from_file(file_contents):
    ...
    issue_patterns = {
        ...
        "TT HSBA Reappointment": r"\bappt( ctt)? hsba\b|\b(1-[8|9]\d{10,11})?.*patch( )?( )?(appointment|appt|ctt).*(1-[8|9]\d{10,11})?\b|\bpatch.*a(p)?(p)?(o)?(i)?(n)?(t)?(m)?(e)?(n)?(t)?.*(\d{2}/\d{2}/\d{4})\s*@\s*(\d{1,2}:\d{2}:\d{2}\s*[ap]m)\b|\b.patch.*a(p)?(p)?(o)?(i)?(n)?(t)?(m)?(e)?(n)?(t)?.*\d{2}/\d{2}/\d{4} \d{1,2}:\d{2}:\d{2} (am|pm)\b|\bpatch.*\d{2}/\d{2}/\d{4}\s\d{1,2}:\d{2}(?::\d{2}\s(?:am|pm)|(?:\s|$))",
        ...
        "TT Duplicated Activity": r"\bc(a)?(n)?c(e)?l dup(licate(d)?)? act(i)?(v)?(i)?(t)?(y)?\b|\bcancel satu a.t(ivity)? a-\d{9,10}\b|\bcancel.*act\b",
