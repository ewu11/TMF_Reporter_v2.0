...
def process_messages_from_file(file_contents):
    ...
    issue_patterns = {
        ...
        "Bypass Extra Port": r"\b(by pass|bypass) (extraport|extrapot|extra port|extra pot)\b|\bby( )?pas(s)? xp\b|\bbantuan bypass kan extraport\b|\bmohon by( )?pas(s)?( act(ivity)?)? (extra( )?port)\b|\bby( )?pas(s)? ext(r)?a( )?port\b|\bby( )?pas(s)? gated extra( )?po(r)?t\b|\bby( )?pas(.)? (e)?xtra( |-)?port\b",
