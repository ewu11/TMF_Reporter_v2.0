...
def process_messages_from_file(file_contents):
    ...
    issue_patterns = {
        ...
        "TT Duplicate CPE SN#": r"\bdup(licate(d)?)? item.*(n(o)?(m)?(b)?(o)?(r)?|n(u)?m(b)?(e)?(r)?) (siri|sn(#)?).*s(a)?m(a|e)\b|\b(n(o)?mb(o)?r|n(u)?mb(e)?r) (siri|sn(#)?).*s(a)?m(a|e)\b|\b(c)?tt.*s(\/)?n dup(licate)?\b"
    }
