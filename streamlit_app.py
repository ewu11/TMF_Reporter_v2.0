...
def process_messages_from_file(file_contents):
    ...
    issue_patterns = {
        ...
        "Resource Management Issue": r"\bsalah zone id\b|\b(manual task|mt) (t(a)?k|x)( )?d(a)?p(a)?t( nak)?( )?assign\b|\ba(s)?(s)?(i)?(g)?(n)? (mt|m(a)?(n)?(u)?(a)?l t(a)?(s)?(k)?)\b|\b(t(a)?k|x)( )?b(o)?l(e)?h upd(a)?(t)?(e)? c(u)?t(i)?\b|\bmasuk((.)?an)?.*ke(.)?d(.)?l(.)?m.*id\b|\bm(.)?s(.)?(.)? (t(.)?k|x) b(.)?l(.)?h.*a(.)?(.)?ig(.)?.*(id|ui|ru)\b|\bteam.*(access|akses)*tm(.)?f(orce)?\b|\bl(.)?p(.)?s t(.)?k(.)?n unlock k(.)?l(.)?(.)?r m(.)?c(.)?m ni(.)?\b",
